...
var SubManager = function(port)
{
    ...
    manager.searchSubtitles = function(imdbId, cb) {
        manager.list = []; //Clear the subtitle list
        console.log("ID: "+imdbId);

        var url = 'http://api.yifysubtitles.com/subs/' + imdbId;
        var mirrorurl = 'http://api.ysubs.com/subs/' + imdbId;
            ...
            if(error || response.statusCode !== 200 || !data || !data.success) {
                request({url:mirrorurl, json: true}, function(error, response, data){
                    if(error || response.statusCode !== 200 || !data || !data.success) {
                        ...
                        var subs = orderSubs(data.subs[imdbId]);
...
var Subtitle = function(lang, iso639, subLink)
{
    ...
    sub.download = function(cb) {
        ...
        request({url: subLink, encoding: null}, function(error, response, data){
